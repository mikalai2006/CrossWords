...
public class UIChampion : UIBase
{
  ...
  private async UniTask DrawLeaderListBlok(LeaderBoard board)
  {
    ...
    var userRankText = Wrapper.Q<Label>("UserRank");
    userRankText.text = board.userRank.ToString();
    ...
    int countLeaderShow = leaderBoard.entries.Count > 10 ? 10 : leaderBoard.entries.Count;
    ...
    for (int i = 0; i < countLeaderShow; i++)
    {
      ...
      var textName = leader.name;
      if (string.IsNullOrEmpty(textName))
      {
        textName = await Helpers.GetLocaledString("noname");
      }
      name.text = textName;
